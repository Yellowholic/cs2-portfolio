# CS2-portfolio ♫⋆｡♪ ₊˚♬ ﾟ.
## Autentico, Jeanne Audrey R. 
## 8 - Dahlia ‧₊˚❀༉‧₊˚.
### Links down below ⋆.𐙚 ̊
---
#### ₊✩‧₊˚౨ৎ˚₊✩‧₊  1st Quarter  ₊✩‧₊˚౨ৎ˚₊✩‧₊

> Hello world code file:
[Hello.py](q1/hello.py)
>
> Distance Activity:
[Distance.py](q1/Autentico_JeanneAudrey_Distance.py)
>
> Score Checker Activity:
- [Score Checker Markdown file](q1/score_checker.md)
- [Score checker code file](q1/score_checker.py)
>
> Input Validation and Verification Activity:
- [Input Validation Markdown file](q1/input_validation.md)
- [Workshop Validator code file](q1/workshop_validator.py)
